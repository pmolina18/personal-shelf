...
- [ ] 1.1 Añadir `podcast = "podcast"` al enum `MediaType` en `backend/schemas/media.py`
- [ ] 1.2 Añadir `SPOTIFY_CLIENT_ID` y `SPOTIFY_CLIENT_SECRET` a `backend/config.py`
- [ ] 1.3 Añadir las variables a `.env.example`
- [ ] 1.4 Añadir las variables a `render.yaml` con `sync: false`
- [ ] 1.5 Verificar con `getDiagnostics` que no hay errores

## Tarea 2 — Spotify Auth helper (Requisito 2)

- [ ] 2.1 Crear `backend/services/spotify_auth.py` con `get_spotify_token()`:
...
- [ ] 2.2 Verificar con `getDiagnostics`

## Tarea 3 — MetadataService: búsqueda de podcasts (Requisito 3)

- [ ] 3.1 Añadir `_search_spotify_metadata()` a `MetadataService`:
...
- [ ] 3.2 Actualizar `search()` para enrutar `media_type="podcast"` al nuevo método
- [ ] 3.3 Verificar con `getDiagnostics`

## Tarea 4 — ImageService: imágenes de podcasts (Requisito 4)

- [ ] 4.1 Añadir `_search_spotify_image()` a `ImageService`:
  - Usa `get_spotify_token()` de `spotify_auth`
  - `GET https://api.spotify.com/v1/search?type=show&q={title}&limit=1`
  - Retorna `images[0].url` o None
- [ ] 4.2 Actualizar `_search_image_url()` para enrutar `media_type="podcast"` al nuevo método
- [ ] 4.3 Verificar con `getDiagnostics`
...
- [ ] 5.1 Añadir CSS custom properties en `App.vue`:
  - `--color-type-podcast`, `--color-type-podcast-bg`, `--color-type-podcast-text`
- [ ] 5.2 Actualizar `MediaCard.vue`: label "Podcast" + borde `.type-podcast`
- [ ] 5.3 Actualizar `FilterBar.vue`: opción "Podcast" en el select de tipo
- [ ] 5.4 Actualizar `MediaForm.vue`: opción "Podcast" en el select de tipo
- [ ] 5.5 Actualizar `ExploreCard.vue`: label "Podcast" en el mapa de tipos
- [ ] 5.6 Verificar con `getDiagnostics` en todos los archivos modificados
...
- [ ] 6.1 Añadir `_fetch_spotify_podcasts()` a `seed_explore.py`
- [ ] 6.2 Integrar en `main()` junto con movies, series y books
